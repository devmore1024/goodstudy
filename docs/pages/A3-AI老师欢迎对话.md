...
- **尺寸**：144 × 144 pt 圆形（full 模式）
- **内容**：小花老师真实卡通头像图片（`/images/teacher.png`）
- **样式**：白色描边 ring-4、大阴影 shadow-2xl、半透明品牌色渐变光晕（blur-2xl）、脉冲光环动画
- **动画**：
...
### 3. 对话气泡
- **位置**：数字人老师形象下方，对话区域内
- **AI 消息**：左侧小花头像（36pt 圆形，带光晕）+ 白色气泡（圆角 16pt，左上方角），投影 `0 2px 12px rgba(0,0,0,0.06)`，左侧三角箭头指向头像
- **用户消息**：右对齐，渐变绿色气泡（brand-light → #d4f5e2），右侧三角箭头
- **内容**：字号 14pt，颜色 #374151，行高 1.5
- **动画**：AI 消息左滑入 + 逐字打字机效果，用户消息右滑入
- **交互**：点击气泡可暂停/继续语音播放
...
### 5. 快捷选项标签
- **位置**：输入栏上方 8pt，水平居中
- **内容**：两个快捷选项胶囊按钮「我是学生」「我是家长」
- **样式**：白色背景，品牌色文字，品牌色/30 边框，圆角全圆，px-20pt py-8pt，shadow-sm
- **交互**：点击直接提交对应选项，同时在聊天区域显示用户气泡
- **显示条件**：仅在 AI 询问身份后等待回答时显示

### 6. 底部固定输入栏（ChatInputBar）
- **位置**：页面底部固定
- **布局**：左侧语音/键盘切换按钮 + 中间输入区 + 右侧扩展按钮
- **文字模式**：圆角输入框（placeholder「输入或语音回答...」）+ 发送按钮（品牌绿圆形，有内容时激活发光）
- **语音模式**：「按住说话」按钮，按住后变为「松开发送」并高亮品牌色
- **切换按钮**：麦克风图标（切换到语音）/ 键盘图标（切换到文字），40pt 圆形
- **背景**：毛玻璃效果（glass），顶部 1px 分隔线
- **显示条件**：在 AI 询问身份并等待回答时显示，其他阶段隐藏

### 7. 背景
- **样式**：温暖渐变背景 page-bg-warm（#FFF8F2→#FFF1E8→#FFFFFF→#F0FAF4）
- **装饰**：半透明渐变装饰圆形（右上角橙色/5 大圆、左下角品牌绿/5 中圆），增加视觉层次
